...
func (s *Server) registerControlRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/github/connection", s.authByMethod(nil, []string{"owner"})(s.handleGitHubConnection))
	mux.HandleFunc("/api/github/app", s.authByMethod(nil, []string{"owner"})(s.handleGitHubApp))
	mux.HandleFunc("/api/github/app/manifest", authOwner(s.handleGitHubAppManifest))
	mux.HandleFunc("/api/github/app/manifest/callback", authOwner(s.handleGitHubAppManifestCallback))
	mux.HandleFunc("/api/github/app/install", authOwner(s.handleGitHubAppInstall))
	mux.HandleFunc("/api/github/app/setup", authOwner(s.handleGitHubAppSetup))
	mux.HandleFunc("/api/github/repos", authOwner(s.handleGitHubRepositories))
