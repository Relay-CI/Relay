...
func wslDefaultDistro() string {
	...
	if err := wslProvisionVesselDistro(); err != nil {
		fmt.Fprintf(os.Stderr, "[vessel] warn: could not provision %s: %v\n", vesselLinuxDistro, err)
		return ""
	}
	return vesselLinuxDistro
}
...
const wslVesselBin = "/usr/local/bin/vessel"

// vesselLinuxDistro is the WSL2 distro name station provisions for itself
// when no user distro (e.g. Ubuntu) is installed.
const vesselLinuxDistro = "vessel-linux"

// wslVesselReady returns true if the Linux station binary is already present.
func wslVesselReady(distro string) bool {
	out, _ := wslRun(distro, "test -x "+wslVesselBin+" && echo yes || echo no")
...
const wslSidecarHashFile = wslVesselBin + ".sidecar-sha256"
...
func wslVesselSidecarCandidates(selfDir string) []string {
	...
	add(os.Getenv("VESSEL_WSL_BIN"))
	add(os.Getenv("RELAY_VESSEL_WSL_BIN"))
	add(filepath.Join(selfDir, "vessel-linux"))
	add(filepath.Join(selfDir, "vessel-linux-"+wslLinuxGoArch()))
	return out
}

func wslVesselSourceDirCandidates(selfDir string) []string {
	seen := map[string]struct{}{}
	out := make([]string, 0, 8)
	add := func(path string) {
		path = strings.TrimSpace(path)
		...
		if _, ok := seen[abs]; ok || !validVesselSourceDir(abs) {
			return
		}
		seen[abs] = struct{}{}
		out = append(out, abs)
	}
	add(os.Getenv("VESSEL_SOURCE_DIR"))
	add(os.Getenv("RELAY_VESSEL_SOURCE_DIR"))
	add(selfDir)
	add(filepath.Join(selfDir, "vessel"))
	add(filepath.Join(selfDir, "..", "vessel"))
	add(filepath.Join(selfDir, "..", "..", "vessel"))
...
func installWSLVesselFromSidecar(distro, binaryPath string) error {
	fmt.Printf("[vessel] installing packaged Linux binary into WSL2 distro %q...\n", distro)
	...
	fmt.Printf("[vessel] installed packaged binary from %s to %s inside WSL2 distro %q\n", binaryPath, wslVesselBin, distro)
	return nil
}

func installWSLVesselFromSource(distro, sourceDir string) error {
	wslSrc := toWSLPath(sourceDir)

	fmt.Printf("[vessel] setting up Linux binary in WSL2 distro %q...\n", distro)

	// Check for Go.
	goPath, _ := wslRun(distro, "which go 2>/dev/null || echo missing")
	if strings.TrimSpace(goPath) == "missing" || goPath == "" {
		fmt.Println("[vessel] Go not found in WSL2 — attempting install (requires sudo)...")
	...
	buildCmd := fmt.Sprintf("cd '%s' && GOOS=linux GOARCH=%s go build -o '%s' . 2>&1",
		wslSrc, wslLinuxGoArch(), wslVesselBin)
	fmt.Printf("[vessel] building: %s\n", buildCmd)
	out, err := wslRunCombined(distro, buildCmd)
	if err != nil {
		return fmt.Errorf("build station in WSL2:\n%s\n%w", out, err)
	}
	fmt.Printf("[vessel] installed at %s inside WSL2 distro %q\n", wslVesselBin, distro)
	return nil
}

// wslInstallStation installs the Linux station binary into WSL2.
...
func wslInstallStation(distro string) error {
	...
	for _, candidate := range wslVesselSidecarCandidates(selfDir) {
		if err := installWSLVesselFromSidecar(distro, candidate); err == nil {
			return nil
		}
	}
	for _, candidate := range wslVesselSourceDirCandidates(selfDir) {
		if err := installWSLVesselFromSource(distro, candidate); err == nil {
			return nil
		}
	}
	return fmt.Errorf(
		"could not install the Linux station binary into WSL2.\n" +
			"Expected either a packaged sidecar like vessel-linux beside station.exe or a valid station source dir.\n" +
			"Fix: rebuild with scripts/build-relayd-with-vessel.ps1, or set RELAY_VESSEL_SOURCE_DIR to the repo's station folder",
	)
}

func wslEnsureVessel(distro string) error {
	self, err := os.Executable()
	if err != nil {
		return fmt.Errorf("resolve executable: %w", err)
	}
	selfDir := filepath.Dir(self)
	sidecars := wslVesselSidecarCandidates(selfDir)
	if len(sidecars) > 0 {
	...
	if wslVesselReady(distro) {
		return nil
	}
	return wslInstallStation(distro)
}
...
func doSpawnWSL(distro string, rec *ContainerRecord, foreground bool, logWriter io.Writer) (int, error) {
	...
	if err := wslEnsureVessel(distro); err != nil {
		return 0, err
	}
	...
	// If the snapshot was synced to WSL2's own store, let the Linux station
	...
	// ContainerRecord reflects the actual 10.88.x.x address. The Linux station
	// will return the same IP when cmdRun calls prepareNetworkEnv (idempotent).
	if rec.NetMode == "bridge" {
		if ipOut, err := wslRun(distro, fmt.Sprintf("%s _net-alloc %s", wslVesselBin, shq(containerNetworkKey(rec.App, rec.ID)))); err == nil {
...
func buildWSLCmd(rec *ContainerRecord, wslDir string, useLinuxImage bool) string {
	...
	if useLinuxImage {
		// Snapshot is in WSL2's own store; pass --image so the Linux station
...
// as a child process of station. Less direct but still runs Linux.
...
// wslProvisionVesselDistro registers a minimal Alpine WSL2 distro named
// "vessel-linux". No pre-existing Linux installation is needed — the Alpine
...
func wslProvisionVesselDistro() error {
	if wslIsRegistered(vesselLinuxDistro) {
		return nil
	}
	...
	stateDir := filepath.Join(appData, "vessel", "distros", vesselLinuxDistro)
	...
	fmt.Printf("[vessel] downloading Alpine minirootfs from %s ...\n", url)
	if err := wslDownloadFile(url, tarPath); err != nil {
		return fmt.Errorf("download Alpine rootfs: %w", err)
	}

	fmt.Printf("[vessel] importing %s into %s ...\n", vesselLinuxDistro, stateDir)
	cmd := exec.Command("wsl.exe", "--import", vesselLinuxDistro, stateDir, tarPath, "--version", "2")
	setCmdHideWindow(cmd)
	out, err := cmd.CombinedOutput()
	_ = os.Remove(tarPath) // cleanup regardless of outcome
	if err != nil {
		return fmt.Errorf("wsl --import %s: %w\n%s", vesselLinuxDistro, err, strings.TrimSpace(string(out)))
	}

	fmt.Printf("[vessel] %s ready\n", vesselLinuxDistro)
	return nil
}
...
func wslSetupRootfs(distro, dest string) error {
	...
	fmt.Printf("[vessel] downloading Alpine rootfs inside WSL2 → %s\n", wslDest)
	cmd := fmt.Sprintf("%s setup-rootfs '%s'", wslVesselBin, wslDest)
	out, err := wslRunCombined(distro, cmd)
	fmt.Print(out)
	if err != nil {
...
const wslKeepaliveScript = `pid=$(cat /tmp/.vessel-keepalive.pid 2>/dev/null); ` +
	`[ -n "$pid" ] && kill -0 "$pid" 2>/dev/null && exit 0; ` +
	`sleep 86400 & echo $! > /tmp/.vessel-keepalive.pid`
...
const stationTaskName = "vessel-wsl-warm"
...
func wslWarm(distro string) {
	fmt.Printf("[vessel] warming WSL2 distro %q...\n", distro)
	if _, err := wslRun(distro, wslKeepaliveScript); err != nil {
		fmt.Fprintf(os.Stderr, "[vessel] warn: keepalive: %v\n", err)
		return
	}
	fmt.Printf("[vessel] WSL2 distro %q is warm — keepalive running for 24 h.\n", distro)

	if wslStartupTaskExists() {
		fmt.Printf("[vessel] startup task %q is already registered.\n", stationTaskName)
		return
	}

	if !isWindowsAdmin() {
		fmt.Println("[vessel] To register the startup task, re-run as Administrator:")
	...
	self, err := os.Executable()
	if err != nil {
		fmt.Fprintf(os.Stderr, "[vessel] warn: resolve executable: %v\n", err)
		return
	}
	if err := wslRegisterStartupTask(self); err != nil {
		fmt.Fprintf(os.Stderr, "[vessel] warn: could not register startup task: %v\n", err)
		return
	}
	fmt.Printf("[vessel] registered startup task %q — WSL2 warms automatically at log on.\n", stationTaskName)
}
