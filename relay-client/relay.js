...
function createSpinner(label) {
  ...
  return {
    start(extra = "") {
      ...
      timer = setInterval(() => render(extra ? `${label} ${extra}` : label), 120);
    },
    update(extra = "") { render(extra ? `${label} ${extra}` : label); },
    stop(success, detail = "") {
      if (timer) clearInterval(timer);
      const elapsed = formatDuration(nowMs() - startedAt);
      const sym = success ? `${c.green}\u2713${c.reset}` : `${c.red}\u2717${c.reset}`;
      const line = `${sym} ${label}${detail ? ` ${c.dim}${detail}${c.reset}` : ""} ${c.dim}(${elapsed})${c.reset}`;
      const pad = Math.max(0, lastLineLen - line.replace(/\x1b\[[0-9;]*m/g, "").length);
      process.stdout.write(`\r${line}${" ".repeat(pad)}\n`);
      lastLineLen = 0;
    },
  };
...
async function runSetupWizard(args, cfgPath) {
  ...
  let socket = null, url = null, token = null;

  if (useSocket) {
    socket = await prompt(
      "Socket path",
      args.socket || process.env.RELAY_SOCKET || path.join(".", "data", "relay.sock")
    );
  } else {
    url   = await prompt("Server URL",  args.url   || process.env.RELAY_URL   || "http://127.0.0.1:8080");
  ...
  // ── 2. App details ──
  console.log("");
  const defaultApp = path.basename(process.cwd()).replace(/[^a-z0-9-]/gi, "-").toLowerCase();
  const app    = await prompt("App name", args.app    || process.env.RELAY_APP    || defaultApp);
...
async function resolveOrSetup(args, { needDeploy = false } = {}) {
  ...
  try {
    ...
    if (!process.stdin.isTTY) {
      ...
      try { resolveTransport(args); } catch (e) { die(e.message); }
      try { if (needDeploy) resolveDeployArgs(args); } catch (e) { die(e.message); }
    }
 ...
 */
function rawRequest(transport, method, apiPath, extraHeaders = {}, body = undefined) {
...
async function apiJSON(transport, method, apiPath, body = undefined) {
  const { status, text } = await rawRequest(transport, method, apiPath, {}, body);
  let json;
...
function putFile(transport, apiPath, absPath) {
  if (transport.kind === "socket") {
    return new Promise((resolve, reject) => {
      ...
      const req = http.request(
        { socketPath: transport.socketPath, method: "PUT", path: apiPath, headers },
        (res) => {
          res.resume();
          res.on("end", () => {
...
function streamLogsTransport(transport, deployId) {
    ...
    return new Promise((resolve) => {
      ...
      function processFrame(frame) {
        ...
        if (eventName === "deploy-status") {
          try { const j = JSON.parse(content); if (j?.status) deployStatus = j.status; }
          catch { deployStatus = content.trim(); }
          return;
        }
        try { const j = JSON.parse(content); if (typeof j?.message === "string") content = j.message; } catch {}
        const text = content.replace(/\r/g, "");
        const isProgress = /(^progress|building|downloading|uploading|extracting|running|compiling|transpiling|installing|bundling)/i.test(text)
          || text.includes("%")
          || (text.length < 120 && (text.endsWith("...") || text.endsWith(".") || text.endsWith("/") || text.includes(" => ")));
        if (isProgress) {
          ...
          if (lastWasStatus) { process.stdout.write("\n"); lastWasStatus = false; lastLineLen = 0; }
          console.log(text);
        }
      }
  ...
  if (transport.kind === "socket") {
    return new Promise((resolve, reject) => {
      const req = http.request(
        { socketPath: transport.socketPath, method: "GET", path: apiPath,
          headers: tokenHeader(transport.token) },
        (res) => {
          if (res.statusCode >= 400) {
            const chunks = [];
            res.on("data", d => chunks.push(d));
            res.on("end", () => reject(new Error(`log stream HTTP ${res.statusCode}: ${Buffer.concat(chunks)}`)));
            return;
          }
  ...
  return fetch(url, { method: "GET", headers: tokenHeader(transport.token) }).then((res) => {
    if (!res.ok) return res.text().then((t) => { throw new Error(`log stream HTTP ${res.status}: ${t}`); });
...
function shouldIgnore(rel) {
  ...
  const ignoreTop = new Set([
    ...
    "coverage", ".relay", "cache", "bin", "obj", "target",
  ]);
  if (ignoreTop.has(top)) return true;
  if (rel === ".relay.json" || rel === ".relayrc" || rel === ".relayrc.json") return true;
  return false;
}
...
async function buildManifest(rootDir) {
  ...
  for (const f of list) {
    ...
    out.push({
      Path: f.rel,
      Size: st.size,
      Mtime: st.mtimeMs ? Math.floor(st.mtimeMs) : Math.floor(st.mtime.getTime()),
      Sha256: sh,
      HashAlgo: "sha256",
      Hash: sh,
    });
...
async function main() {
  ...
  if (cmd === "init") {
    ...
    if (hasFlags) {
      ...
      const cfgOut = {};
      if (args.socket || process.env.RELAY_SOCKET) cfgOut.socket = args.socket || process.env.RELAY_SOCKET;
      if (args.url    || process.env.RELAY_URL)    cfgOut.url    = args.url    || process.env.RELAY_URL;
      if (args.token  || process.env.RELAY_TOKEN)  cfgOut.token  = args.token  || process.env.RELAY_TOKEN;
      ...
      // Interactive wizard.
      if (!process.stdin.isTTY) die("No flags given and stdin is not a TTY. Pass --app, --url, --token etc.");
  ...
  if (cmd === "projects") {
    ...
    try {
      ...
      if (!projects.length) { info("No projects found"); process.exit(0); }
      for (const p of projects) {
        console.log(`${c.bold}${p.name}${c.reset}`);
        for (const env of (p.envs || [])) {
          const status = env.latestDeploy?.status ?? "idle";
          const sc = statusColor(status);
          const location = env.stopped ? "offline" : (env.public_host || env.host_port ? (env.public_host || `port:${env.host_port}`) : "no url");
          console.log(`  ${c.dim}${env.env}/${env.branch}${c.reset}  ${sc}${status}${c.reset}  ${c.dim}${location}${c.reset}`);
        }
      }
  ...
  if (cmd === "list") {
    ...
    try {
      ...
      if (!rows.length) { info("No deploys found"); process.exit(0); }
      const idW = 10, appW = 18, envW = 8, brW = 16, stW = 10;
  ...
  if (cmd === "status") {
    ...
    try {
      const deploys = await apiJSON(transport, "GET", "/api/deploys");
      const match   = deploys.find((d) => d.app === resolved.app && d.env === resolved.env && d.branch === resolved.branch);
      if (!match) { warn(`No deploys found for ${resolved.app}/${resolved.env}/${resolved.branch}`); process.exit(0); }
      const appState = await apiJSON(transport, "GET", `/api/apps/config?app=${encodeURIComponent(resolved.app)}&env=${encodeURIComponent(resolved.env)}&branch=${encodeURIComponent(resolved.branch)}`);
      const appStopped = Boolean(appState?.Stopped ?? appState?.stopped);
      const sc = statusColor(match.status);
      console.log(`${c.bold}${match.app}${c.reset}  ${match.env}/${match.branch}`);
      console.log(`  ID       ${c.dim}${match.id}${c.reset}`);
      console.log(`  Status   ${sc}${match.status}${c.reset}`);
      console.log(`  App      ${appStopped ? `${c.yellow}offline${c.reset}  ${c.dim}(image ready, start manually)${c.reset}` : `${c.green}online${c.reset}`}`);
      if (match.created_at)  console.log(`  Created  ${c.dim}${new Date(match.created_at).toLocaleString()}${c.reset}`);
      if (match.started_at)  console.log(`  Started  ${c.dim}${new Date(match.started_at).toLocaleString()}${c.reset}`);
      if (match.ended_at)    console.log(`  Ended    ${c.dim}${new Date(match.ended_at).toLocaleString()}${c.reset}`);
      if (match.preview_url) console.log(`  URL      ${c.cyan}${match.preview_url}${c.reset}`);
      if (match.error)       console.log(`  Error    ${c.red}${match.error}${c.reset}`);
    } catch (e) { die(e.message); }
    process.exit(0);
  }
  ...
  if (cmd === "rollback") {
    ...
    try {
      const res = await apiJSON(transport, "POST", "/api/deploys/rollback", {
        app: resolved.app, env: resolved.env, branch: resolved.branch,
      });
  ...
  if (cmd === "secrets") {
    ...
    if (!sub || sub === "list") {
      try {
        const secrets = await apiJSON(transport, "GET", `/api/apps/secrets${qs}`);
        if (!secrets.length) { info("No secrets configured"); process.exit(0); }
    ...
    if (sub === "add") {
      ...
      if (!key || !value) die("Usage: relay secrets add --key KEY --value VALUE --app ... --env ... --branch ...");
      try {
        await apiJSON(transport, "POST", "/api/apps/secrets", {
          app: resolved.app, env: resolved.env, branch: resolved.branch, key, value,
        });
        ok(`Secret ${key} saved`);
      } catch (e) { die(e.message); }
      process.exit(0);
    ...
    if (sub === "rm" || sub === "remove") {
      ...
      if (!key) die("Usage: relay secrets rm --key KEY --app ... --env ... --branch ...");
      try {
        await apiJSON(transport, "DELETE", `/api/apps/secrets${qs}&key=${encodeURIComponent(key)}`);
  ...
  if (cmd === "plugin") {
    ...
    if (sub === "list") {
      try {
        const plugins = await apiJSON(transport, "GET", "/api/plugins/buildpacks");
        if (!Array.isArray(plugins) || !plugins.length) { info("No buildpack plugins installed"); process.exit(0); }
        for (const p of plugins) {
          const kind = p.plan?.kind ? ` ${c.dim}(${p.plan.kind})${c.reset}` : "";
    ...
    if (sub === "install") {
      ...
      let plugin;
      try { plugin = JSON.parse(await fsp.readFile(path.resolve(ref), "utf8")); }
      catch (e) { die(`Failed to read plugin file: ${e.message}`); }
      try {
        const res = await apiJSON(transport, "POST", "/api/plugins/buildpacks", plugin);
    ...
    if (sub === "remove" || sub === "uninstall") {
      ...
      try {
        await apiJSON(transport, "DELETE", `/api/plugins/buildpacks/${encodeURIComponent(name)}`);
        ok(`Removed buildpack plugin: ${name}`);
      } catch (e) { die(e.message); }
      process.exit(0);
    }

    die("Unknown plugin sub-command. Supported: list, install <file>, remove <name>");
  }
  // ── agent ──────────────────────────────────────────────────────────────────
  if (cmd === "agent") {
    const sub = args._[1];
    ...
    function platformAsset() {
      const p = process.platform; const a = process.arch;
      if (p === "linux"  && a === "x64")   return { tarball: "relay-linux-amd64.tar.gz",  zip: false };
      if (p === "linux"  && a === "arm64") return { tarball: "relay-linux-arm64.tar.gz",  zip: false };
      if (p === "win32"  && a === "x64")   return { tarball: "relay-windows-amd64.zip",   zip: true  };
    ...
    async function installAgentRelease(version) {
      ...
      const ext = process.platform === "win32" ? ".exe" : "";
      await fsp.writeFile(path.join(installDir, ".relay-version"), version, "utf8");

      console.log("");
      ok(`relayd   ${c.dim}\u2192${c.reset} ${path.join(installDir, "relayd" + ext)}`);
      ok(`station  ${c.dim}\u2192${c.reset} ${path.join(installDir, "station" + ext)}`);
  ...
  const hasHostPortOverride = Object.prototype.hasOwnProperty.call(args, "host-port");
  const hasServicePortOverride = Object.prototype.hasOwnProperty.call(args, "service-port");
  const hasPublicHostOverride = Object.prototype.hasOwnProperty.call(args, "public-host");
  const mode = hasModeOverride ? String(args["mode"]).trim() : "";
  const hostPort = hasHostPortOverride ? Number(args["host-port"]) || 0 : 0;
  const servicePort = hasServicePortOverride ? Number(args["service-port"]) || 0 : 0;
  const publicHost = hasPublicHostOverride ? String(args["public-host"]).trim() : "";

  const rootDir      = path.resolve(dir);
  const cfg          = await readJSONIfExists(path.join(rootDir, "relay.config.json"));
  const localDefaults = await readJSONIfExists(path.join(rootDir, ".relay.json"));
  ...
  planSpinner.start();
  const plan = await apiJSON(transport, "POST", `/api/sync/plan/${sessionId}`, { files: manifest });
  planSpinner.stop(true, `need=${(plan.need||[]).length} delete=${(plan.delete||[]).length}`);
  ...
  if (need.length) {
    ...
    for (let i = 0; i < need.length; i++) {
      ...
      process.stdout.write(`   [${i + 1}/${need.length}] ${c.dim}${rel}${c.reset}\n`);
      const uploadPath = `/api/sync/upload/${sessionId}?path=${encodeURIComponent(rel)}`;
      await putFile(transport, uploadPath, abs);
    }
    ok(`uploaded ${need.length} file${need.length === 1 ? "" : "s"} in ${formatDuration(nowMs() - uploadStartedAt)}`);
  }

  if (del.length) {
    info(`delete ${del.length} stale path${del.length === 1 ? "" : "s"}`);
    await apiJSON(transport, "POST", `/api/sync/delete/${sessionId}`, { paths: del });
  }
  ...
  if (!noStream) {
    ...
    const duration      = finalDeploy.started_at && finalDeploy.ended_at
      ...
      : totalElapsed;
    const previewURL = finalDeploy.preview_url || (hostPort ? `http://127.0.0.1:${hostPort}` : "");

    if (String(finalStatus).toLowerCase() === "success") {
      if (appStopped) {
        ok(`deploy built in ${c.bold}${duration}${c.reset} and kept offline  ${c.dim}(stream ${streamElapsed}, total ${totalElapsed})${c.reset}`);
        if (finalDeploy.image_tag) console.log(`  image    ${c.dim}${finalDeploy.image_tag}${c.reset}`);
        console.log(`  state    ${c.yellow}offline${c.reset}  ${c.dim}ready for 'relay start' when you want it live${c.reset}`);
      } else {
        ok(`deploy ready in ${c.bold}${duration}${c.reset}  ${c.dim}(stream ${streamElapsed}, total ${totalElapsed})${c.reset}`);
        if (previewURL) console.log(`  ${c.cyan}${previewURL}${c.reset}`);
      }
    } else {
      const sc = statusColor(finalStatus);
      warn(`deploy finished with status ${sc}${finalStatus}${c.reset} after ${totalElapsed}`);
      process.exitCode = 1;
    }
  } else {
